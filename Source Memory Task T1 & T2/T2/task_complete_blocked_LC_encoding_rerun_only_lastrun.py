...
expInfo = {'participant': '', 'session': '001', 'group': '', 'university': '', 'run': ''}
...
filename = _thisDir + os.sep + 'data/%s_run%s_%s_%s' %(expInfo['participant'], expInfo['run'],expName, expInfo['date'])

# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='C:\\Users\\lacherep\\Documents\\Source Memory Task_updated\\Source Memory Task_updated\\task_complete_blocked_LC_encoding_rerun_only_lastrun.py',
...
while continueRoutine:
    ...
    if scanner_pulse.status == STARTED and not waitOnFlip:
        theseKeys = scanner_pulse.getKeys(keyList=['5', 't'], waitRelease=False)
